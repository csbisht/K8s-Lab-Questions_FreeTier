...
if [ -d "/opt/K8sLab/Lab2" ]; then
sudo chown -R ubuntu:ubuntu /opt/K8sLab
else
sudo mkdir -p /opt/K8sLab/Lab2
sudo chown -R ubuntu:ubuntu /opt/K8sLab	
fi	

###make executable to all scripts
sudo chmod +x "$HOME"/K8s-Lab-Questions_FreeTier/CKA_Lab2/qsn_check/*.sh

qsncheck () {
clusterconf=`ls "$HOME"/K8s-Lab-Questions_FreeTier/kubeconfig |cut -d'.' -f1`
for y in `echo ${clusterconf}`
do
source "$HOME"/K8s-Lab-Questions_FreeTier/CKA_Lab2/qsn_check/Q"${qsn_no}".sh "${y}"
done
if [ "${out3}" -gt 0 ]; then
echo -e "\n\n"	
...
questionlst=( `ls $HOME/K8s-Lab-Questions_FreeTier/CKA_Lab2/questions` )

labrun () {
clear
echo -e "\n\n\n\n"
qsnread=`cat $HOME/K8s-Lab-Questions_FreeTier/CKA_Lab2/questions/Q"${qsn_no}".md | fmt`
printf "${BOLD}${CYAN}${qsnread}${NONE}"
echo -e "\n\n"
echo -e "${BOLD}${confirm}${NONE}"
echo -e "${BOLD}${nextq}${NONE}"
...
do
qsn_no=`expr ${i} + 1`	
if [ -f "${HOME}/K8s-Lab-Questions_FreeTier/CKA_Lab2/qsn_check/Q${qsn_no}_runfirst.sh" ]; then
chmod +x "${HOME}/K8s-Lab-Questions_FreeTier/CKA_Lab2/qsn_check/Q${qsn_no}_runfirst.sh"
"${HOME}"/K8s-Lab-Questions_FreeTier/CKA_Lab2/qsn_check/Q"${qsn_no}"_runfirst.sh
labrun
else
labrun
fi	
