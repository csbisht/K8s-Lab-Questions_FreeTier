...
if [ ${out1} = 0 ]; then
checkpodmnt=`/usr/bin/kubectl --kubeconfig=$HOME/K8s-Lab-Questions_FreeTier/kubeconfig/"$1".config get pod "$podname""$clstnum" -o jsonpath='{.spec.containers[*].volumeMounts[?(@.mountPath == "/data")]}' |grep -w "/data"`
out2="$?"
if [ ${out2} = 0 ]; then
checkmntsize=`/usr/bin/kubectl --kubeconfig=$HOME/K8s-Lab-Questions_FreeTier/kubeconfig/"$1".config get pvc pvc"$clstnum"-data -o jsonpath='{.spec.resources.requests.storage}' |grep 50Mi`
out1="$?"
if [ ${out1} = 0 ]; then
