...
if [ "${out3}" = 0 ]; then
checklabel=`/usr/bin/kubectl --kubeconfig=$HOME/K8s-Lab-Questions_FreeTier/kubeconfig/"$1".config get  pods "$podname""$clstnum" --show-labels |awk '{print $6}' |grep -w "$labelrm"`
out1="$?"	
