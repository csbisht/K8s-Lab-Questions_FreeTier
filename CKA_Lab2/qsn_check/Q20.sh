...
gethost_master=`/usr/bin/kubectl --kubeconfig=$HOME/K8s-Lab-Questions_FreeTier/kubeconfig/"$1".config get node "$1"-controlplane -o=jsonpath='{.status.addresses[?(@.type=="Hostname")].address}'`

checkfile=`cat "$filepath"/"$filename""$clstnum"_List |grep -w "$gethost_master"`
out3="$?"
if [ ${out3} = 0 ]; then
getmaster_ip=`/usr/bin/kubectl --kubeconfig=$HOME/K8s-Lab-Questions_FreeTier/kubeconfig/"$1".config get node "$1"-controlplane -o=jsonpath='{.status.addresses[?(@.type=="InternalIP")].address}'`
checkfile=`cat "$filepath"/"$filename""$clstnum"_List |grep -w "$getmaster_ip"`
out2="$?"

if [ ${out2} = 0 ]; then
gethost_controlplane=`/usr/bin/kubectl --kubeconfig=$HOME/K8s-Lab-Questions_FreeTier/kubeconfig/"$1".config get node "$1"-controlplane -o=jsonpath='{.status.addresses[?(@.type=="Hostname")].address}'`
checkfile=`cat "$filepath"/"$filename""$clstnum"_List |grep -w "$gethost_controlplane"`
out1="$?"

if [ ${out1} = 0 ]; then
getcontrolplane_ip=`/usr/bin/kubectl --kubeconfig=$HOME/K8s-Lab-Questions_FreeTier/kubeconfig/"$1".config get node "$1"-controlplane -o=jsonpath='{.status.addresses[?(@.type=="InternalIP")].address}'`
