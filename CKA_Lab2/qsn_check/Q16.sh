...
if [ "${out3}" = 0 ]; then
getpodsec=`/usr/bin/kubectl --kubeconfig=$HOME/K8s-Lab-Questions_FreeTier/kubeconfig/"$1".config get pod "$podname" -o jsonpath='{.spec.securityContext}' |grep -w "$grpname" |grep -w "2000" |grep -w "$asusername" |grep -w "1000" |grep -w "$asgrpname" |grep -w "3000"`
out1="$?"
